...
namespace MVC.Controller.Game
{
    public class GameStartState : IState
    {
        ...
        public GameStartState(GameController gameController)
        {
        ...
        public void Enter()
        {
            ...
            _countdownTimer = 5f;
            _gameController.ShowStartPanel(true);
            _gameController.ShowEndPanel(false);

            // 초기 카운트다운 텍스트 설정
            _gameController.UpdateCountdownText(Mathf.Ceil(_countdownTimer).ToString());
        }
        ...
        public void Execute()
        {
            ...
            if (displayNumber > 0)
            {
                _gameController.UpdateCountdownText(displayNumber.ToString());
            }
            else
            {
                _gameController.UpdateCountdownText("START!");
            }

            // 카운트다운 완료 시 게임 플레이 상태로 전환
            if (_countdownTimer <= 0f)
            {
                _gameController.ChangeToGamePlaying();
            }
        }

        public void Exit()
        {
            _gameController.ShowStartPanel(false);
    ...
    class GamePlayingState : IState
    {
        ...
        public GamePlayingState(GameController gameController)
        {
            _gameController = gameController;
        }
        ...
        public void Execute()
        {
            ...
            _gameController.UpdatePlayingTimerText(timeText);
        ...
        private void TimeUp()
        {
            ...
            // 시간 초과로 게임 종료 (패배 처리)
            _gameController.ChangeToGameEndWithResult("Time Up!\nDefeat!", false);
        }
    }
    ...
    public class GameEndState : IState
    {
        ...
        public void Enter()
        {
            ...
            _gameController.ShowStartPanel(false);
            _gameController.ShowEndPanel(true);
            _gameController.UpdateEndText(_resultText);
